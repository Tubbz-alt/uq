...
#cat("\n eeeeeeee, res.obs.now is ", res.obs.now, "\n")
#cat("\n ncomp.dis is ", ncomp.dis, "\n")
#cat("\n nbeta0 is ", beta.now[1,], "\n")
...
#cat("\n kkkkkkkkkkkk, dhat.mat is ", dhat.mat, "\n")
#cat("\n ffffffffffff, res.obs.now is ", res.obs.now, "\n")
...
BSSANOVA.calibration <- function(y, X, ys, Xs, Ts, cat.theta=rep(0,ncol(Ts)), pi.theta=NULL, X.lim=NULL, T.lim=NULL, transform=NULL, inv.transform=NULL, int.order=NULL, P=c(25,8,6,4), max.ord=P+1, P.Sigma=NULL, v.Sigma=NULL, P.Ups=NULL, v.Ups=NULL, P.Gamma=NULL, v.Gamma=NULL, P.Lambda=NULL, v.Lambda=NULL, N.grid=500, N.mcmc=6000, nplot=20, nback=5000, theta.now=NULL, alpha.now=NULL, beta.now=NULL, Lambda.now=NULL, Gamma.now=NULL, Ups.now=NULL, Sigma.now=NULL, theta.prop.sd=1, include2="default", include3=NULL, include4=NULL, mu.Sigma=NULL, mu.Ups=NULL, mu.Gamma=NULL, mu.Lambda=NULL, N.init=0, theta.names=NULL, cat.names=NULL, x.names=NULL, y.names=NULL, pow=rep(1,ncol(y))){
...
#Phi..<<-Phi
  ...
  Nreplica = 1;
  theta.replica <- randomLHS(Nreplica,K)
#cat("theta_replica is ", theta.replica,"\n")  
for(ll in 1:Nreplica)  
{
 ...
 ## Initialize posterior objects
  ...
  if(is.null(P.Lambda))
    ...
    cat("\nP.Lambda is ", P.Lambda);
    cat("\nmu.Lambda is ", mu.Lambda);
    cat("\nv.Lambda is ", v.Lambda);

    cat("\nP.Gamma is ", P.Gamma);
    cat("\nmu.Gamma is ", mu.Gamma);
    cat("\nv.Gamma is ", v.Gamma);
...
#  cat("alpha intial", alpha.now[1:7,1],"\n");
#  cat("beta intial", beta.now[1:7,1],"\n");

    

#  if(is.null(file_replica.names))
#    file_replica.names <- paste("results_replica_",1:Nreplica,".Rdata",sep="")

 ## Begin MCMC ##
cat("\n")
  #reset theta value following Latin 
#  theta.now = theta.replica[ll,]
#  theta[1,] = theta.now
...
cat("\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\bIteration", it, "out of", N.mcmc)
   ...
   ## Update missing residuals
#cat("\n nnnnnnn res.obs.now is ", res.obs.now, "\n");
    res.obs.now <- update.resid(res.obs.now, Sigma.now, ind.na.obs)


   ## Update alphas
#cat("\n mmmmmmmmmm res.obs.now is ", res.obs.now, "\n");
   ...
   ## Update betas
#cat("\n ccccc res.obs.now is ", res.obs.now, "\n");

#cat("\n llllllllll, dhat.mat is ", dhat.mat, "\n")
...
#cat("\n bbbb res.obs.now is ", res.obs.now, "\n");
#cat("\n beta.now is ", beta.now, "\n");
...
#cat("\nLambda.now size ", ncol(Lambda.now));  
#cat("\ncomp.em size ", ncomp.em);  
...
#    Ups.now <- rUps.cond(res.sim.now, P.Ups, v.Ups)
   ...
   ## Update Sigma
#cat("\n aaa res.obs.now is ", res.obs.now, "\n");
...
#cat("\nP.Sigma is ", P.Sigma);
#cat("\nv.Sigma is ", v.Sigma);
#cat("\nSigma.now is ", Sigma.now,"\n");
#cat("\nUps.now is ", Ups.now,"\n");
#cat("\ndim.Lambda.now is ", dim(Lambda.now),"\n");
#cat("\nLambda.now is ", Lambda.now,"\n");
...
#cat("\n aaaaa: yc.now ", yc.now, "\n");
#cat("\n aaaaa: ytru.now ", ytru.now, "\n");
#cat("\n dim(X.emd) ", dim(X.emd));
#cat("\n dim(alpha.now) ", dim(alpha.now));
#cat("\n dim(X.dis) ", dim(X.dis));
#cat("\n dim(beta.now) ", dim(beta.now));
#cat("\n dim(res.obs.now) ", dim(res.obs.now));
#cat("\n res.obs.now is ", res.obs.now, "\n");
#cat("\n beta.now is ", beta.now, "\n");
...
#cat("\ndim(X.emd.p) is ", dim(X.emd.p), "\n") 
#cat("\ndim(alpha.now) is ", dim(alpha.now), "\n") 
#cat("\ndim(X.dis) is ", dim(X.dis), "\n") 
#cat("\ndim(beta.now) is ", dim(beta.now), "\n") 
#cat("\n ncol(X.dis) is ", ncol(X.dis), "\n")
#cat("\n bbbb: yc.now ", yc.now,"\n");
#cat("\n bbbb: res.obs.prop ", res.obs.prop,"\n");
...
#cat("pi.prop, pi.now ", pi.prop, pi.now, "\n")
#cat("dnow.g.p, dp.g.now ", dnow.g.p, dp.g.now, "\n")
        MH.ratio <- exp(like.prop+pi.prop+dnow.g.p-like.now-pi.now-dp.g.now)
      
#cat("\n k is ", k, "theta.p is ", theta.p[k], " MH.ratio is ", MH.ratio, 
#          " exp() ", (like.prop+pi.prop+dnow.g.p-like.now-pi.now-dp.g.now), "\n");
      }
...
#cat("\ntheta.now is ",theta.now," accept.count is ", accept.count[it, ], "\n")
...
#cat("c(1,col.ind.em[[j]])", c(1,col.ind.em[[j]]));
#cat("\n");
        }
...
#err_tmp <- X.dis[,c(1,col.ind.dis[[j]])]%*%beta[i,c(1,col.ind.dis[[j]]),m]
            lines(x.j[ord.j],y.hat[ord.j],col=26,cex=.7)
            lines(x.j[ord.j],y.sim[ord.j],col=51,cex=.5)
#cat("x.j is ", x.j[ord.j], "\n")
#cat("x.j is ", x.j[ord.j], "\n")
#cat("X[] is ", X, "\n")
#cat("ind.curves ", ind.curves, "\n");
#cat("i is ", i, " ord.j is ", ord.j, "\n");
          }
          points(x.j[!ind.na.m], yd[!ind.na.m], pch='x', col=47, cex=1.5)  
...
plot.fitted.all.output <- function(ans.calibration, post.ind=NULL, inputs=NULL, outputs=NULL, ncurves=500, nplot=ncurves, ylim=NULL, legend.loc="topright", transpose=FALSE, y.names=NULL,x.names=NULL, nx.curve=50, mar=NULL, cex.leg=.8){
...
for(j in 1:J.plot){
  ...
  for(mp in 1:M.plot){
    ...
    if(is.null(ylim))
      ...
      yd.lim <- ylim[m,]
    plot(x.j, yd, main=paste("Emulator+Discrepancy, ",y.names[m],sep=""), ylim=yd.lim, pch='x', col=0, ylab="",xlab="")
    title(ylab=y.names[m], xlab=x.names[inputs[j]],line=2)  
    legend(legend.loc[m], legend=c("Emulator Mean","Emulator Reals","Emu + Dis Mean", "Emu + Dis Reals", "Observed Data"), lty=c(1,1,1,1,1), pch=c(NA,NA,NA,NA,'X'), lwd=c(2,.5,2,.5,1), col=c(sim.col[1:2],dis.col[1:2],1), cex=cex.leg)
...
#ans_test <- get.all.X(cbind(X,matrix(theta.hat,nrow=nrow(X),ncol=K,byrow=TRUE)), t.grid, Phi, P=P, max.ord=max.ord, include2, include3, include4, cat.vars=c(rep(0,ncol(X)), cat.theta))
...
#cat("y.hat", y.hat,"\n");
#cat("d.hat", d.hat,"\n");
#cat("y_test", ans_test$X%*%alpha.hat, "\n");
  if(trans.scale){
    for(m in 1:M){
      y.hat[,m] <- inv.transform[[m]](y.hat[,m], pow[m])
